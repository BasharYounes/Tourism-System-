...
class CompleteFlightReservationController extends Controller
{
   ...
   public function __construct(Request $request)
   {
    ...
    {
        $reservation = new Complete_Flight_Reservation();
        ...
        if (!$reserv_room) {
            $message = $this->translator->translate("It Have Not Room For The Number Of People ... ");
        ...
        $complete_flight = Complete_Flight::find($id);
        $user_id = auth()->user()->id;
        $true = Complete_Flight_Reservation::where('user_id',$user_id)
        ->where('complete_flight_id',$id)
        ->first();
        if($true!= null)
        {
        ...
        if( $complete_flight->price <= $cash_user->cash)
        {
          ...
          $reservation->complete_flight_id = $id;
    ...
    {
        $reservation_flight = Complete_Flight_Reservation::find($id);

        $complete_flight_updated_id = $request->input('complete_flight_id');
        $complete_flight_updated = Complete_Flight::find($complete_flight_updated_id);
        ...
        $flight = Complete_Flight::find($reservation_flight->complete_flight_id);
        ...
        if(Carbon::today() < $previous)
       {
            ...
            $reservation_flight->complete_flight_id = $complete_flight_updated_id;
            ...
            $reservation_flight ,
            "complete_flight_updated" => $complete_flight_updated,
            "flight"=>$flight
            ],
             200);
        }
    ...
    {
        $complete_flight = Complete_Flight_Reservation::find($id);
        ...
        $flight = Complete_Flight::find($complete_flight->complete_flight_id);
    ...
    {
       return Complete_Flight::where('name','like','%'.$request->name.'%')->get();
    }
}